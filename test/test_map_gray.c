...
/* exporting a plain Tiff - Section */
    ...
    sql = "SELECT RL2_WriteSectionTiff('main', ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ret = sqlite3_prepare_v2 (sqlite, sql, strlen (sql), &stmt, NULL);
    if (ret != SQLITE_OK)
	return 0;
    sqlite3_reset (stmt);
...
/* exporting a Tiff + Worldfile - Section */
    ...
    sql = "SELECT RL2_WriteSectionTiffTfw('main', ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ret = sqlite3_prepare_v2 (sqlite, sql, strlen (sql), &stmt, NULL);
    if (ret != SQLITE_OK)
	return 0;
    sqlite3_reset (stmt);
...
/* exporting a PNG/JPEG/TIFF/PDF image */
    ...
    sql =
	"SELECT RL2_GetMapImageFromRaster('main', ?, ST_Buffer(?, ?), 512, 512, 'default', ?, '#ffffff', 0, 80)";
    ret = sqlite3_prepare_v2 (sqlite, sql, strlen (sql), &stmt, NULL);
    if (ret != SQLITE_OK)
	return 0;
    sqlite3_reset (stmt);
